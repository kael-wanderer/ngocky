...
- **Dashboard** – Summary cards with search and time filter, grouped category selector (Personal / Family / Hobby) with All/None per group and click-order section rendering, overdue feed, Pin Items card (always visible, independent of category filter), and category-based panels for Goals, Tasks, Projects, Ideas, Healthbook, Housework, Calendar, Expenses, Assets, Keyboard, Funds, and Learning
...
- **Tasks** – Standalone tasks with repeat rules (`Daily`, `Weekly`, `Monthly`, `Quarterly`), payment-task support, sharing, dashboard pinning, drag reorder, optional reminders; list view includes a filter bar (keyword search, due date defaulting to This Month, type, priority, status), sortable column headers with up/down arrows, always-visible colour-coded action icons (green Done / orange Reopen, amber Pin, grey Edit, blue Duplicate, red Delete), and inline badges below the title for due date, notification, and auto-expense
- **Projects** – Shared boards with kanban + list view, project task types (`Task`, `Bug`, `Feature`, `Story`, `Epic`), priorities, deadlines, drag-and-drop status updates, and per-item sharing; board list supports keyword search + time/type/status filters and sortable columns; board task view includes a filter bar (keyword search, type, priority, status, Hide Done toggle that collapses Done/Archived columns in kanban), sortable list columns, Bell icon notification badge (grey/red) with date+time pills, and always-visible colour-coded action icons
...
- **Analytics** – Charts and summaries for project items, standalone tasks, goals, calendar, housework, expenses, assets, healthbook, learning, ideas, keyboard, and funds with time-aware filters and module tabs (single/multi select); filter panel has keyword search on the left side; Analytics visibility follows `User Settings > Desktop Features`
...
- **Dashboard**: grouped category selector (Personal / Family / Hobby) with All/None buttons per group; categories include Keyboard, Funds, and Healthbook; sections render in click order; Pin Items card moved to same row as Overdue and is independent of category filter; search input added left of time filter; compact Auto-refresh toggle; removed Status filter
- **Healthbook logs**: table view with column headers (Type, Date, Location, Doctor, Cost, Alert, Actions); Duplicate action icon; notification reminders via shared `NotificationFields` component; Bell badge and purple notification pills in both list and grid views; notification fields saved via fixed Zod validator
- **Analytics**: Healthbook tab added to module selection; keyword search added to the left of filter controls; filter panel title changed from "🔍 Analytics Configuration" to "Filters"
- **Keyboard**: Filters header (Filter icon + "Filters" text) added above the filter bar consistent with Tasks page
- **Notification fields fix**: Zod validators for HealthLog (`createHealthLogSchema` / `updateHealthLogSchema`) now include all notification fields; previously they were silently stripped before reaching the route handler
